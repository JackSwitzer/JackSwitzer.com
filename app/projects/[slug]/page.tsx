...
export default function ProjectDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <section>
      {/* Breadcrumb */}
      <nav className="text-xs font-mono uppercase tracking-wide mb-8">
        <Link href="/" className="text-[var(--muted)] hover:text-[var(--accent)] transition-colors">
          Home
        </Link>
        <span className="mx-2 text-[var(--border)]">/</span>
        <Link href="/projects" className="text-[var(--muted)] hover:text-[var(--accent)] transition-colors">
          Projects
        </Link>
        <span className="mx-2 text-[var(--border)]">/</span>
      ...
      <header className="border-b border-[var(--border)] pb-6 mb-6">
        <div className="eyebrow mb-2">{project.type}</div>
        <h1 className="text-3xl font-bold leading-tight mb-3">{project.name}</h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {project.github && (
            <a
              ...
              className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-[var(--ink)] text-[var(--paper)] text-sm font-mono uppercase tracking-wide hover:opacity-90 transition-opacity"
            >
          ...
          {project.link && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-2 py-0.5 border border-[var(--border)] text-sm font-mono uppercase tracking-wide hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors"
            >
          ...
          <h2 className="mb-4 flex items-center gap-2">
            <span
              className={project.slug === "sol"
                ? "px-3 py-1.5 text-xl font-bold uppercase tracking-wide bg-[var(--success)] text-[var(--paper)]"
                : "tag tag-success"
              }
            >
              {project.slug === "sol" ? "How Sol Works" : "Key Accomplishments"}
            </span>
          </h2>
